      ...
      <div className="prism-site-footer__inner">
        <div className="prism-site-footer__products" data-prism="site-footer-products">
          {prismProducts.map((product) => {
            const current = product.id === site;
            return (
              <Link
                key={product.id}
                href={product.url}
                className="prism-site-footer__product-link"
                aria-current={current ? 'page' : undefined}
              >
                <span className="prism-site-footer__product" data-prism="site-footer-product" data-product={product.id} data-current={current ? 'true' : undefined}>
                  ...
                  {current ? <span className="prism-visually-hidden">Current site</span> : null}
                </span>
              </Link>
            );
          })}
        ...
        {columns?.length ? (
          <nav className="prism-site-footer__columns" aria-label="Footer">
            {columns.map((column) => (
              <div key={column.title} className="prism-site-footer__column">
                <h2>{column.title}</h2>
                {column.links.map((link) => <Link key={`${link.url}:${link.label}`} href={link.url}>{link.label}</Link>)}
              </div>
            ))}
          </nav>
        ) : null}
        <div className="prism-site-footer__base">
          {social?.length ? (
            <div className="prism-site-footer__social" aria-label="Social links">
              {social.map((entry) => (
                <Link key={`${entry.url}:${entry.label}`} href={entry.url} aria-label={entry.label}>
