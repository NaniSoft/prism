...
- **License for the public npm packages** — packages carry `license: UNLICENSED` (bootstrap placeholder); npm refuses a real publish without one. Choosing MIT / Apache-2.0 / proprietary is a decision; must land before the first actual publish (npm org arrives with ticket 07). Surfaced by the repo bootstrap.
